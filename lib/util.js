...
module.exports.parseMockfileList = parsemockfiles;

function parsemockfiles() {
  const dir = "mock_file/";
  const files = fs.readdirSync(dir);
  var tempmockfilelist = [];
  files.forEach(function (filename, index) {
    if (filename.endsWith(".json")) {
      const path = dir + filename;
      const jsonStr = JSON.parse(readfileSync(path));
      tempmockfilelist.push(jsonStr);
    }
  });
  mockfilesList = tempmockfilelist;
  return mockfilesList;
};

module.exports.getMockfileList = function () {
  return mockfilesList;
};

module.exports.watchdir = function () {
  var chokidar = require('chokidar')
  var watcher = null
  var ready = false
  ...
  if (!watcher) {
    watcher = chokidar.watch(__dirname + '/../mock_file')
  }
  watcher
