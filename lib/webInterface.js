...
class webInterface extends events.EventEmitter {
  ...
  getServer() {
    ...
    app.post('/savemockdata', (req, res) => {
      if (req.body === undefined) {
        return;
      }
      const reqBody = JSON.stringify(req.body, null, 2).replace(/\\/g, '');
      var filename = req.body.request.method + req.body.request.path.replace(/\//g, '_');
      console.log("req is:" + filename);
      fs.writeFile(__dirname + '/../mock_file/' + filename +'.json', reqBody, function (err) {
        if (err) {
          return res.json({
            status: 'failed',
          });
